...
from healthrag.rag.embedder import get_biomedical_embedder

DEFAULT_HOST = os.getenv("QDRANT_HOST", "localhost")
DEFAULT_PORT = int(os.getenv("QDRANT_PORT", "6333"))


def collection_name(collection: str, layer: str | None = None) -> str:
    """Name one vector collection per literature collection and corpus layer.
    ...
    """
    version = "medcpt_v1"
    base = f"literature_{collection.lower()}_{version}"
    return base if layer is None else f"{base}_{layer}"


def build_vector_store(documents: list[Document], collection: str, host: str = DEFAULT_HOST,
    ...
    return QdrantVectorStore.from_documents(
        ...
        collection_name=collection_name(collection, layer),
        **connection,
        force_recreate=overwrite,
    )


def load_vector_store(collection: str, host: str = DEFAULT_HOST,
...
def list_collections(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT) -> list[str]:
