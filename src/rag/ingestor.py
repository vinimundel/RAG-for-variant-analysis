...
from src.literature.corpus_layers import UNLAYERED, load_layer_map
from src.literature.pubmed_records import load_cache
...
def layer_map_for_gene(data_root: Path, gene_symbol: str) -> dict[str, str]:
    """Load the curated ``pmid -> corpus_layer`` map produced by corpus layering."""
    gene = gene_symbol.upper()
    return load_layer_map(data_root / "data" / "input" / gene / "evidence" /
                          f"{gene.lower()}_corpus_layers.csv")


def literature_metadata_for_gene(data_root: Path, gene_symbol: str) -> dict[str, dict]:
    """Load bibliographic provenance once for metadata carried into every chunk."""
    gene = gene_symbol.upper()
    cache = load_cache(data_root / "data" / "input" / gene / "evidence" /
...
def extract_tei_for_gene(data_root: Path, gene_symbol: str, *,
                         grobid_url: str = "http://localhost:8070",
                         workers: int = 1) -> dict:
    """Incrementally materialize the GROBID cache without retaining the corpus.

    This is deliberately separate from ``load_pdfs_for_gene``.  Extraction can
    ...
    gene = gene_symbol.upper()
    candidates = [data_root / "data" / "input" / gene / "literature" / "pdfs",
                  data_root / "data" / "input" / gene / "evidence" / "pdfs"]
    ...
    tei_dir = data_root / "data" / "output" / gene / "rag" / "tei"
    ...
    checkpoint = tei_dir.parent / f"{gene.lower()}_grobid_extraction_manifest.json"
...
def load_pdfs_for_gene(data_root: Path, gene_symbol: str, *,
                       grobid_url: str = "http://localhost:8070",
    ...
    grobid_healthcheck(grobid_url)
    tei_dir = data_root / "data" / "output" / gene / "rag" / "tei"
    manifest_path = data_root / "data" / "input" / gene / "evidence" / f"{gene.lower()}_literature_evidence.csv"
    source_by_pmid = {}
    ...
    layers = layer_map_for_gene(data_root, gene)
    literature_metadata = literature_metadata_for_gene(data_root, gene)
    def process(pdf: Path) -> list[Document]:
        ...
        pmid = _pmid(pdf.name)
        metadata = {"pmid": pmid, "gene": gene, "source_file": pdf.name,
    ...
    output.extend(load_pubmed_abstracts_for_gene(
        data_root, gene, fulltext_pmids, layers, literature_metadata))
    if not output:
        raise RuntimeError("PDF ingestion produced zero non-empty chunks")
    return output


def load_pubmed_abstracts_for_gene(data_root: Path, gene_symbol: str,
                                   fulltext_pmids: set[str],
                                   layers: dict[str, str] | None = None,
                                   literature_metadata: dict[str, dict] | None = None) -> list[Document]:
    path = (data_root / "data" / "input" / gene_symbol.upper() / "evidence" /
            "abstracts" / "pubmed_abstracts.jsonl")
    if not path.exists():
        return []
    layers = layer_map_for_gene(data_root, gene_symbol) if layers is None else layers
    literature_metadata = (literature_metadata_for_gene(data_root, gene_symbol)
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        base = Document(page_content=content, metadata={
            "pmid": pmid, "gene": gene_symbol.upper(), "title": title,
