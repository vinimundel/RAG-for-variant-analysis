...
class PDFDownloader:
    ...
    def resolve_pubmed_metadata(self, pmid: str) -> Dict[str, Optional[str]]:
        ...
        # 2. NCBI ID Converter Fallback if pmcid or doi missing
    ...
    def get_pdf_url_from_openalex(self, pmid: str = None, doi: str = None) -> Optional[str]:
        ...
        try:
            ...
            res = requests.get(url, headers={"User-Agent": "mailto:researcher@epistasis_predictor.org"}, timeout=10)
            if res.status_code == 200:
                data = res.json()
    ...
    def get_pdf_url_from_unpaywall(self, doi: str) -> Optional[str]:
        ...
        try:
            url = f"https://api.unpaywall.org/v2/{doi}?email=researcher@epistasis_predictor.org"
